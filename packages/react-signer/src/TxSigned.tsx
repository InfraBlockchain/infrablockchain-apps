...
async function signAndSend (queueSetTxStatus: QueueTxMessageSetStatus, currentItem: QueueTx, tx: SubmittableExtrinsic<'promise'>, pairOrAddress: KeyringPair | string, options: Partial<any>): Promise<void> {
...
async function signAsync (queueSetTxStatus: QueueTxMessageSetStatus, { id, txFailedCb = NOOP, txStartCb = NOOP }: QueueTx, tx: SubmittableExtrinsic<'promise'>, pairOrAddress: KeyringPair | string, options: Partial<any>): Promise<string | null> {
  txStartCb();
...
async function extractParams (api: ApiPromise, address: string, options: Partial<any>, getLedger: () => Ledger, setQrState: (state: QrState) => void): Promise<['qr' | 'signing', string, Partial<any>]> {
...
function TxSigned ({ className, currentItem, isQueueSubmit, queueSize, requestAddress, setIsQueueSubmit }: Props): React.ReactElement<Props> | null {
  ...
  const [signedOptions, setSignedOptions] = useState<Partial<any>>({});
  ...
  const [vote, setVote] = useState<PotVote | undefined>();
  const [assetPayment, setAssetPaymemt] = useState<AssetPaymentInterface | undefined>();
          ...
          {(isBusy && flags.isQr)
            ...
            : (
              <>
                ...
                {!currentItem.payload && (
                  <AssetPayment onChange={setAssetPaymemt} />
                )}
                {!currentItem.payload && (
                  <Vote onChange={setVote} />
                )}
                {!currentItem.payload && (
                  <FeePayer onChange={setFeePayer} />
                )}
                {!isSubmit && (
                  <SignFields
